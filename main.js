...
let foxes = localStorage.foxes ? JSON.parse(localStorage.foxes) : [['light', 'Robson', 10, false, 0], ['normal', 'Ronald', 15, false, 0], ['heavy', 'Romary', 15, false, 0]]
...
function update() {
    if (menu == 'result') {
        ...
        if(points < 3 && enemyPoints < 3 && mx > canvas.width / 2 - 60 && my > canvas.height - 100 && mx < canvas.width / 2 + 60 && my < canvas.height){
            ...
            mx = -1
            my = -1
            if (result == 'victory')localStorage.money = parseInt(localStorage.money)+1
        }
    }
    if (menu == 'zooming') {
        zoom += 0.0125
