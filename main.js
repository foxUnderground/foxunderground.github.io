...
let shopLogo = new Image()
shopLogo.src = 'https://i.imgur.com/2Ki4FqL.png'
let sellings = [
    {
        type: 'fox',
        price: 3,
...
class JudoFox {
    constructor(weight, name, muscularMass, isTraining, time) {
    ...
    training(interval) {
        this.trainingStart = interval || Date.now()
        interval = Date.now()-interval || 120000
        this.isTraining = true
        setTimeout(() => { this.muscularMass += 1; this.isTraining = false }, interval);
    }
    draw(i) {
...
let foxes = JSON.parse(localStorage.foxes) || [['light', 'Robson', 10, false, 0], ['normal', 'Ronald', 15, false, 0], ['heavy', 'Romary', 15, false, 0]]
console.log(localStorage.foxes, typeof foxes, foxes)
foxes = foxes.map(e=>new JudoFox(e[0], e[1], e[2], e[3], e[4]))
let menu = 'start'
let money = parseInt(localStorage.money) || 0
...
function update() {
    if (menu == 'result') {
        ...
        if(points < 3 && enemyPoints < 3 && mx > canvas.width / 2 - 60 && my > canvas.height - 100 && mx < canvas.width / 2 + 60 && my < canvas.height){
            ...
            mx = -1
            my = -1
            localStorage.money = parseInt(localStorage.money)+1
        }
    }
    if (menu == 'zooming') {
        zoom += 0.0125
    ...
    if (menu == 'battle') {
        ...
        localStorage.foxes = JSON.stringify(foxes.map(e=>[e.weight, e.name, e.muscularMass, e.isTraining, e.trainingStart]))
        ctx.drawImage(menuBackground, 0, 0, menuBackground.width, menuBackground.height, 0, -150, 640, 640)
        card(canvas.width / 2 - (CARD_WIDTH * 3 + CARD_SPACING * 2) / 2, canvas.height / 2 - CARD_HEIGHT / 2, 'BATTLE', battleLogo, () => { menu = 'battle' })
        card(canvas.width / 2 - (CARD_WIDTH * 3 + CARD_SPACING * 2) / 2 + CARD_WIDTH + CARD_SPACING, canvas.height / 2 - CARD_HEIGHT / 2, 'FOXES', foxesLogo, () => { menu = 'fox' })
        card(canvas.width / 2 - (CARD_WIDTH * 3 + CARD_SPACING * 2) / 2 + CARD_WIDTH * 2 + CARD_SPACING * 2, canvas.height / 2 - CARD_HEIGHT / 2, 'SHOP', shopLogo, () => { menu = 'shop' })
    }
    else if (menu == 'fox') {
        ctx.drawImage(menuBackground, 0, 0, menuBackground.width, menuBackground.height, 0, -150, 640, 640)
